  ...
  const announcements = [
    {
      ...
      excerpt: 'Pendaftaran bantuan sosial PKH untuk tahap II telah dibuka dari tanggal 15 Januari hingga 31 Januari 2025.',
    ...
    },
    {
      ...
      excerpt: 'Informasi jadwal pelayanan administrasi kependudukan untuk bulan Februari 2025 dengan sistem antrian online.',
      image: 'https://images.pexels.com/photos/7688460/pexels-photo-7688460.jpeg?auto=compress&cs=tinysrgb&w=600&h=300&fit=crop',
      date: '2025-01-12',
    ...
    },
    {
      ...
      excerpt: 'Laporan lengkap progres pembangunan infrastruktur desa tahun 2024 dan rencana 2025.',
      image: 'https://images.pexels.com/photos/1051838/pexels-photo-1051838.jpeg?auto=compress&cs=tinysrgb&w=600&h=300&fit=crop',
      date: '2025-01-10',
    ...
    },
    {
      ...
      excerpt: 'Persiapan festival budaya tahunan dengan berbagai kegiatan seni dan budaya lokal.',
              ...
              >
                {categories.map(category => (
                  <option key={category.id} value={category.id}>
                    {t(`info.${category.id === 'semua' ? 'all_categories' : category.id}`)}
                  </option>
                ))}
              </select>
            </div>
            ...
            {filteredAnnouncements.length > 0 && (
              <div className="mb-8">
                <h2 className="text-2xl font-bold text-gray-900 mb-6">
                  {selectedCategory === 'semua' ? t('info.latest_info') : 
                   t(`info.${selectedCategory === 'semua' ? 'latest_info' : selectedCategory}`)}
                </h2>
                ...
                <div className="space-y-3">
                  {schedules.map((schedule, index) => (
                    <div key={index} className="border-l-4 border-green-500 pl-3">
                      <h4 className="font-medium text-sm">{schedule.service}</h4>
                      <p className="text-xs text-gray-600">{schedule.day}</p>
                      <p className="text-xs text-gray-600">{schedule.time}</p>
                      <p className="text-xs text-green-600">{schedule.location}</p>
                    </div>
                  ))}
                </div>
              </CardContent>
            ...
            <Card className="bg-red-50 border-red-200">
              <CardContent className="p-6">
                ...
                <div className="space-y-2 text-sm">
                  <div>
                    <p className="font-medium text-red-800">{t('info.police')}</p>
                  </div>
                  <div>
                    <p className="font-medium text-red-800">{t('info.fire_dept')}</p>
                  </div>
                  <div>
                    <p className="font-medium text-red-800">{t('info.ambulance')}</p>
                  </div>
                  <div>
                    <p className="font-medium text-red-800">{t('info.village_office')}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
